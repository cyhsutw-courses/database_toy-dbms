...
public class UI {
	...
	private void testAPI(String command) throws Exception {
		if (command.compareToIgnoreCase("Metadata") == 0)
			...
			BufferedReader buff = new BufferedReader(new FileReader(arr[1]));
			...
			String[] _sqls = file.split(";\n");
			int size = _sqls.length;
			...
			/*
			 * for (String sql : sqls) { long l = System.currentTimeMillis();
			 * Chiisai.planner().execute(sql); long spendms =
			 * System.currentTimeMillis() - l; System.out.println(">>> total " +
			 * spendms + " ms"); }
			 */
			String tb = arr[1].replaceAll(".sql", ".tbl");
			// System.out.println(tb);
			Chiisai.fMgr().updateNumberOfRecordsBy(tb, size);
			RecordFile.reset();
		} else {
			long l = System.currentTimeMillis();
			Chiisai.planner().execute(command);
